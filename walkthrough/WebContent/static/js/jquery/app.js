...
function nav_page_height() {
	...
	if($('#announcementAngAppId').is(':visible')){  
		setHeight = $('#announcementMain').height();
	}
...
if($('#announcementAngAppId').is(':visible')){ 
	$('#announcementMain').resize(function() {
		nav_page_height();
		check_if_mobile_width();
	})
}
...
function runAllCharts() {
	...
	if ($.fn.sparkline) {
		...
		$('.sparkline').each(function() {
			...
			if (sparklineType == 'bar') {
				//Filter the drawing for TOP LEVEL Proficiency Charts
				if ($this.attr('name') == 'proficiencyChart') {
					return;
				}
			...
			//MINPROFROW CHART for new DASHBOARD
			if (sparklineType == 'profrow') {

				var sparklineHeight = $this.data('sparkline-height') || '20px', sparklineWidth = $this.data('sparkline-width') || '90px', thisLineColor = $this.data('sparkline-line-color') || $this.css('color') || '#0000f0', thisLineWidth = $this.data('sparkline-line-width') || 1, thisFill = $this.data('fill-color') || '#c0d0f0', thisSpotColor = $this.data('sparkline-spot-color') || '#f08000', thisMinSpotColor = $this.data('sparkline-minspot-color') || '#ed1c24', thisMaxSpotColor = $this.data('sparkline-maxspot-color') || '#f08000', thishighlightSpotColor = $this.data('sparkline-highlightspot-color') || '#50f050', thisHighlightLineColor = $this.data('sparkline-highlightline-color') || 'f02020', thisSpotRadius = $this.data('sparkline-spotradius') || 4.5;
				thisChartMinYRange = $this.data('sparkline-min-y') || 'undefined', thisChartMaxYRange = $this.data('sparkline-max-y') || 'undefined', thisChartMinXRange = $this.data('sparkline-min-x') || 'undefined', thisChartMaxXRange = $this.data('sparkline-max-x') || 'undefined', thisMinNormValue = $this.data('min-val') || 'undefined', thisMaxNormValue = $this.data('max-val') || 'undefined', thisNormColor = $this.data('norm-color') || '#c0c0c0', thisDrawNormalOnTop = $this.data('draw-normal') || false;
			...
			//COMPOSITE: PROFICIENCY ROW
			if (sparklineType == 'compositeprofrow') {
				...
				$this.sparkline(sparklineValue, {
					...
					type : 'line',
					spotRadius : 0,
					tooltipSuffix : '% Participation',
					spotColor : '',
					minSpotColor : '#1c1a54',
					maxSpotColor : '#00aeef',
					highlightSpotColor : '#8dc63f',
					highlightLineColor : '#8dc63f',
     ...
     * Circle stats
     */
    $('.circleStatsChart').each(function () {
    	var circleColor = $(this).data('circle-color');
    	...
    	var circleStatsContainerDiv = $(this).parents('.circleStatsMdItem');
    	circleStatsContainerDiv.css('color', circleColor).find('i, span').css('color', circleColor);
    });
	...
	$('.easy-pie-chart').each(function() {
		var $this = $(this);
		var barColor 	= $this.data('pie-color') || '#eeeeee'; 
		var trackColor	= $this.data('pie-track-color') || '#eeeeee';
		var size 		= parseInt($this.data('pie-size')) || 25;
		var rotate		= $this.data('pie-rotate') || -90;
		$this.easyPieChart({
			barColor   : barColor,
...
function pageSetUp() {
    ...
    if ($.device === "desktop" && !(window.navigator.maxTouchPoints > 0)){
		// is desktop(excluding windows tablet)
		
	    /* bind "mouseenter" "mouseleave" events to popover div. */
		...
		/*
		 * Elements with attribute [data-popover] cotain clickable object, like report links, 
		 * in order to enable mouseenter event, they should not be hidden at once.
		 */
		$("[data-popover=true]").popover({trigger: 'hover', placement: $this.data('placement'), delay: {show: 50, hide: 400}});
		...
		// is mobile(including windows tablet)
		
		var triggerFortablet = "click";
		if(navigator.userAgent.indexOf("Android") >= 0){
		    triggerFortablet = "hover"
		}
		
		// activate tooltips
		$("[rel=tooltip]").tooltip({
		    trigger : triggerFortablet
		});
		...
		/* elements with attribute "[data-popover]" cotain clickable object, like report links. */
         $("[data-popover=true]").popover({trigger: triggerFortablet, placement: $this.data('placement')});
		...
		/* dismiss existing popover div(s) on iPad when expected*/
		function hidePopover(e){
	        /* if anywhere outside a popover div is touched, hide all the existing popover div.*/
	        $('[rel=popover-hover],[rel=popover],[data-popover=true]').each(function () {
	    ...
	    if(navigator.userAgent.indexOf("iPad") >= 0){
	        $('body').off('touchend', hidePopover).on('touchend', hidePopover);
        }
	    if(window.navigator.maxTouchPoints > 0){
            $('body').off('click', hidePopover).on('click', hidePopover);
        }
	}
}

// Get FusionCharts object by type and instance id, if not found, create object
function getFusionChartsInstance(type, id, width, height){
    var chart;
    
    if("ActiveXObject" in window){
        /* special for IE: 
         * check if the browser is IE browser.
         * Get a new FusionCharts to avoid unknown errors on IE9/10/11
         */
         
         /* dispose existing fusion chart with the same id */
         try {
            var existingChart = FusionCharts.getObjectReference(id);
            if(existingChart && existingChart != null){
        ...
        try {
	        chart = FusionCharts.getObjectReference(id);
	        if(chart.chartType().toLowerCase() != type.toLowerCase()){
...
/* load series YTD graphs 
 * sparkDiv : HTML div for include canvas.
 * avgscoreData : array of avgscore data  
 * participationData : array of participation data
 * monthJsonStr : json string of month information 
 * isDisableTooltip : is disable tooltip, true or false.
*/
function loadSeriesYTDGraphs(sparkDiv, avgscoreData, participationData, monthJsonStr, isDisableTooltip){
  
  /* get some para from data-option attribute. */
  var sparklineHeight           =   sparkDiv.data('sparkline-height') || '80px';
  var sparklineWidth            =   sparkDiv.data('sparkline-width') || '200px';
  var thisLineColor1            =   sparkDiv.data('sparkline-color-top') || '#FFFFFF';
  var thisLineColor2            =   sparkDiv.data('sparkline-color-bottom') || '#000000';
  var thisSpotColor             =   sparkDiv.data('sparkline-spot-color') || '#f08000';
  var thisMinSpotColor          =   sparkDiv.data('sparkline-minspot-color-top') || '#ed1c24';
  var thisMaxSpotColor          =   sparkDiv.data('sparkline-maxspot-color-top') || '#f08000';
  var thisHighlightSpotColor    =   sparkDiv.data('sparkline-highlightspot-color-top') || '#50f050';
  var thisHighlightLineColor    =   sparkDiv.data('sparkline-highlightline-color-top') || '#f02020';
  var thisFillColor             =   sparkDiv.data('sparkline-fillcolor-top') || 'transparent';
  participationData             =   participationData || sparkDiv.data('sparkline-partici-val');
  avgscoreData                  =   avgscoreData || sparkDiv.data('sparkline-avgscore-val');
  monthJsonStr                  =   monthJsonStr || sparkDiv.data('sparkline-month-val');
  ...
  var sparklineConfig = {
      ...
      tooltipValueLookups: monthJsonStr
  };
  if(isDisableTooltip){
    sparklineConfig.disableTooltips = true;
    sparklineConfig.disableHighlight = true;
  }
  /* call spark line API "sparkline()" method for draw month information. */
  sparkDiv.sparkline(participationData, sparklineConfig );

  var sparklineAvgscoreConfig = {
      type : 'line',
      spotRadius : 5,
      tooltipSuffix : '% Correct',
      spotColor : '',
  ...
  if(isDisableTooltip){
    sparklineAvgscoreConfig.disableTooltips = true;
    sparklineAvgscoreConfig.disableHighlight = true;
  }
   
  /* call sparkline() method for draw avgscore line. */
  sparkDiv.sparkline(avgscoreData, sparklineAvgscoreConfig);

  var sparklineParticiConfig = {
      type : 'line',
      spotRadius : 5,
      tooltipSuffix : '% Participation',
      spotColor : '',
  ...
  if(isDisableTooltip){
    sparklineParticiConfig.disableTooltips = true;
    sparklineParticiConfig.disableHighlight = true;
  }
  
  /* call sparkline() method for draw participation line. */
  sparkDiv.sparkline(participationData, sparklineParticiConfig);
}

/* resize Dashboard window will re-load TYD graphs. */
function reloadSeriesYTDgraphs(isDisableTooltip){
    $('.sparkline').each(function() {
        
        /* get spark line type "ytdgraphs" that means will load series YTD graphs. */
        var sparklineType = $(this).data('sparkline-type') || 'notype';
        
        //COMPOSITE: performance ROW
        if (sparklineType == 'ytdgraphs') {
            loadSeriesYTDGraphs($(this),null,null,null,isDisableTooltip);
        } 
...
/* show processing bar */
function showProcessingDivForDashboard(id){
    $("#"+id).show();
}

/* hide processing bar */
function hideProcessingDivForDashboard(id){
    $("#"+id).hide();
}
...
/* Draw top level bar chart */
function reloadTopLevelProficiencyChart(){

	if ($.fn.sparkline) {
		$("div[name='proficiencyChart']").each(function() {
			var $this = $(this);
			var sparklineType = $this.data('sparkline-type') || 'bar';
			// BAR CHART
			if (sparklineType == 'bar') {
...
/* Draw MSLine  chart */
function drawMSLineChart(reportJSON){
	if(null != reportJSON && reportJSON != ''){
		var reportsJSON = jQuery.parseJSON(reportJSON);
	 	for(var seriesId in reportsJSON){
	 		var jsonStr = reportsJSON[seriesId];
	 		var chartMath = getFusionChartsInstance("MSLine", "MSLineChart_"+seriesId);
			chartMath.setJSONData(jsonStr);
	    	chartMath.render("MSLChart_"+seriesId);
	 	}
	}
}


/* Active dataTable for dashboard */
function activeDataTableForDashboard(id){
	jQuery("table[id^='dt_basic_']").each(function() {
		activeDataTables(this.id);
	});
}

/* Active dataTable */
function activeDataTables(id){
	if(jQuery('#'+id).get(0)){
		/* Active data table */
		var table = jQuery('#'+id).dataTable({
			...
			"dom": 'T<"clear">lfrtip',
			"tableTools": {			
				"sSwfPath": "/suite/body/SmartAdmin/js/plugin/table-tools/media/swf/copy_csv_xls_pdf.swf",
				"aButtons": [
								{
									"sExtends": "pdf",
									"sFileName": "InCompletedStudents.pdf",
									"bHeader": true
								},
								{
									"sExtends": "xls",
									"sFileName": "InCompletedStudents.xls",
									"bHeader": true
								},
								{
									"sExtends": "print"
...
/* Draw Column 3D chart */
function drawColumn3DChart(id, charJSON){
	if(null != charJSON && charJSON != ''){
 		var chartColumn3D = getFusionChartsInstance("column3d", "column3dChart");
		chartColumn3D.setJSONData(charJSON);
    	chartColumn3D.render(id);
	}
}

/* page set up for print */
function pageSetUpForDiv(pdfDiv) {

    /* remove pie chart canvas */
    $(pdfDiv).find('.sparkline').find('canvas').remove();
    
    /* get pie chart data */
    $(pdfDiv).find('.sparkline').each(function(){
          var pieval = $(this).attr("pieval");
          $(this).append(pieval);
     });
    
    /* load all charts */    
    runAllChartsForDiv(pdfDiv); 
    
    /* remove button events */
    $(pdfDiv).find("a").removeAttr("href").removeAttr("onclick").addClass("disabled").css("opacity",1).css("filter","alpha(opacity=100)");
    $(pdfDiv).css("overflow","auto").css('background','white');
}

function runAllChartsForDiv(pdfDiv) {
    ...
    $(pdfDiv).find('.sparkline').each(function() {
        ...
        //COMPOSITE: performance ROW
        if (sparklineType == 'ytdgraphs') {
           loadSeriesYTDGraphs($(this),null,null,null,true);
        }
        
    });

    $(pdfDiv).find('.easy-pie-chart').each(function() {
        var $this = $(this);
